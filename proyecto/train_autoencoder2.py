...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 10 05:03:58 2016
...
import numpy as np
def train_autoencoder(data,data2,out,encoding_dims,activation,out_act):
#    input_data = Input(shape=(data.shape[1],))
    ...
    for i in range(20):
        if(i<19):
            model.fit(data[i*division:(i+1)*division,:],data2[i*division:(i+1)*division,:],nb_epoch=100,shuffle=True,
                    validation_split=24*1./data[i*division:(i+1)*division,:].shape[0],batch_size=1,
                    callbacks=[early_stopping],verbose=True)
        else:
            model.fit(data[i*division:,:],data2[i*division:,:],nb_epoch=100,shuffle=True,
    ...
    predictor.fit(data,out,nb_epoch=100,shuffle=False,
                    validation_split=24*5./data.shape[0],batch_size=1,
